...
export const App = (): JSX.Element => {
  ...
  const handleNewRegistration = async (e: React.FormEvent) => {
    ...
    const newUser: User = {
      ...
      email: email,
      uniqueId: email, 
      password: password, // Store the validated password
      role: role,
      displayName: name,
    ...
    setSuccessMessage(`Registration successful for ${name}! Please login. (Email simulation in console)`);
    setAuthView('login');
  };
 ...
 const handlePreRegistrationSubmit = async (e: React.FormEvent) => {
    ...
    const newPendingUser: PendingUser = {
        ...
        password: preRegistrationForm.password, // Store validated password
  ...
  const handleInitiateApprovePendingUser = (pendingUser: PendingUser) => { 
    ...
    if (pendingUser.email && pendingUser.password) {
        infoMsg += "Email and password were provided by the user. Review other details and role. You may override the password if needed.";
    } else if (pendingUser.email) {
        infoMsg += "Email was provided by the user. Please set a password, review other details and role.";
    } else {
