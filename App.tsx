...
const App = (): JSX.Element => {
  ...
  const handlePreRegistrationSubmit = (e: React.FormEvent) => { 
      e.preventDefault(); 
      clearMessages(); 
      const { email, uniqueId, displayName, referringAdminId, isReferralLinkValid } = preRegistrationForm;

      if (!isReferralLinkValid || !referringAdminId) { 
          setError("Invalid pre-registration attempt. Please use a valid link from an administrator."); 
          return; 
      } 
      if (!email.trim() || !uniqueId.trim() || !displayName.trim()) { 
          setError("Email, your Desired System ID, and Display Name are required."); 
          return; 
      } 
      if (!/\S+@\S+\.\S+/.test(email)) {
        setError("Please enter a valid email address.");
      ...
      if (users.some(u => u.uniqueId === uniqueId) || pendingUsers.some(pu => pu.uniqueId === uniqueId)) { 
      ...
      if (users.some(u => u.email === email) || pendingUsers.some(pu => pu.email === email)) {
        setError("This email address has already been used or is pending approval. Please choose a different one or login.");
        return;
      }
      ...
      setSuccessMessage("Your pre-registration has been received. An administrator will review it. You will be able to log in after an admin approves your account and sets up your password."); 
      setAuthView('login');
  ...
  const handleInitiateApprovePendingUser = (pendingUser: PendingUser) => { setEditingUserId(null); setApprovingPendingUser(pendingUser); setUserForm({ ...initialUserFormData, email: pendingUser.email, uniqueId: pendingUser.uniqueId, displayName: pendingUser.displayName, referringAdminId: pendingUser.referringAdminId, role: 'user', }); clearMessages(); setInfoMessage(`Reviewing pending user: ${pendingUser.displayName} (ID: ${pendingUser.uniqueId}, Email: ${pendingUser.email}). Please complete their profile, set a password, and confirm/assign a role.`); };
