...
exports.sendPreRegistrationSubmittedToUserEmail = async (email, displayName, adminDisplayName) => {
  const templateParams = {
    to_email: email,// Placeholder for email service
 ...
 * @param companyName Optional name of the company if the user is an admin creating a new site.
 */
export const sendWelcomeRegistrationEmail = async (email: string, displayName: string, role: Role, companyName?: string): Promise<void> => {
  ...
  if (role === 'admin' && companyName && companyName.trim() !== '') {
    body += `\n    Your new site for "${companyName}" is ready.`;
  }
